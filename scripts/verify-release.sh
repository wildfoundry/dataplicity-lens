...
[[ $(find "$directory" -maxdepth 1 -type f -name 'dataplicity-lens-v*-*.tar.gz' | wc -l) -ge 8 ]] || { echo "expected eight Linux and macOS target archives" >&2; exit 1; }
[[ $(find "$directory" -maxdepth 1 -type f -name 'dataplicity-lens_*.deb' | wc -l) -ge 3 ]] || { echo "expected three Debian packages" >&2; exit 1; }
...
for archive in "$directory"/dataplicity-lens-v*-*.tar.gz; do
  ...
  for binary in lens lens-top lens-services lens-logs lens-disk lens-net lens-system lens-health; do
